...
quadrado).
  ...
  A estratégia para esse exercício foi a mesma do exercício 1, mas com o input do tamanho do tabuleiro antes
  de se iniciar o jogo. 

  Utiliza um loop while na função principal iniciar_jogo()
...
def configurar_tabuleiro():
  '''Função inicial para criar o tabuleiro com o tamanho desejado com as casas vazias (" ")'''
  n = receber_tamanho_tabuleiro()

  global tabuleiro
  tabuleiro = [[' '] * n] * n

def receber_tamanho_tabuleiro():
  '''Recebe um número para o tamanho do tabuleiro da partida. Mínimo 3. '''
  while True:
    try:
      n = int(input("Qual será o tamanho do jogo? min. 3 "))

      if (n < 3):
        print("O tamanho precisa ser no mínimo 3. ")
        continue
...
print(colorido("*JOGO DA VELHA NxN*", cores['HEADER']) + "\n")
